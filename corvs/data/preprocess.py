...
def loc_to_spd(loc: ArrayLike, freq_in_hz: float, resol_per_px: float) -> NDArray[np.floating]:
    vec = np.diff(loc, axis=0)
    dist = resol_per_px * linalg.norm(vec, axis=1)
    spd = freq_in_hz * dist
    return spd

def loc_to_spd_v2(loc: ArrayLike, freq_in_hz: float, resol_per_px: float) -> NDArray[np.floating]:
    vec = np.gradient(loc, axis=0)
    dist = resol_per_px * linalg.norm(vec, axis=1)
    spd = freq_in_hz * dist
    return spd
...
@typing.overload
def to_convex(ang: NDArray[np.integer]) -> NDArray[np.float64]:
    ...

@numba.njit(cache=True)
def to_convex(ang: NDArray[np.floating | np.integer]) -> NDArray[np.floating]:
    return (ang + np.pi) % (2 * np.pi) - np.pi

def loc_to_turn_rate(loc: ArrayLike, freq_in_hz: float) -> NDArray[np.floating]:
    vec = np.diff(loc, axis=0)
...
def loc_to_turn_rate_v2(loc: ArrayLike, freq_in_hz: float) -> NDArray[np.floating]:
    vec = np.gradient(loc, axis=0)
    dir = np.arctan2(vec[:, 1], vec[:, 0])
    ang = np.gradient(np.unwrap(dir))
    ang_vel = freq_in_hz * ang
    return ang_vel

@typing.overload
def sync(time_1: ArrayLike, val_1: ArrayLike, time_2: ArrayLike, val_2: ArrayLike, /, freq_in_hz: float, *, kind: str = "linear") -> tuple[NDArray[np.float64], NDArray[np.float64], NDArray[np.float64]]:
    ...

@typing.overload
def sync(time_1: ArrayLike, val_1: ArrayLike, time_2: ArrayLike, val_2: ArrayLike, time_3: ArrayLike, val_3: ArrayLike, /, freq_in_hz: float, *, kind: str = "linear") -> tuple[NDArray[np.float64], NDArray[np.float64], NDArray[np.float64], NDArray[np.float64]]:
    ...

@typing.overload
def sync(*times_and_vals: ArrayLike, freq_in_hz: float, kind: str = "linear") -> tuple[NDArray[np.float64], ...]:
    ...
...
def pad(seqs: torch.Tensor | list[torch.Tensor], tgt_len: int, batch_first: bool = False, pad_val: float = 0.0, pad_side: Literal["left", "right"] = "right") -> torch.Tensor:
    """
