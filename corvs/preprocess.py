...
def loc_to_spd(loc: ArrayLike, freq_in_hz: float, resol_per_px: float) -> NDArray[np.float64]:
...
def loc_to_turn_rate(loc: ArrayLike, freq_in_hz: float) -> NDArray[np.float64]:
    vec = np.diff(loc, axis=0)
