...
from corvs import CorVSNetPredictor, CorVSPredictDataModule, utils
...
def infer(data_path: PathLike, param_path: PathLike, weight_path: PathLike, traj_track_id: int, sensor_worker_id: int) -> None:
    torch.set_float32_matmul_precision("high")
    hparams = OmegaConf.load(param_path)

    datamodule = CorVSPredictDataModule(data_path, traj_track_id, sensor_worker_id, hparams, datetime(2024, 10, 3, hour=11, minute=30, tzinfo=utils.jst), datetime(2024, 10, 3, hour=12, tzinfo=utils.jst))
...
if __name__ == "__main__":
    ...
    infer(args.data_path, args.param_path, args.weight_path, args.traj_track_id, args.sensor_worker_id)
